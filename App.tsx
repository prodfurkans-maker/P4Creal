    ...
    try {
      ...
      setSessions(prev => prev.map(s => 
        s.id === currentSessionId ? { 
          ...s, 
          messages: [...s.messages, {
            id: Date.now().toString(), role: 'assistant', content: "Hızlı bir bağlantı hatası oldu, tekrar dene.", timestamp: Date.now()
          }] 
        } : s
      ));
    } finally {
  ...
  return (
    <div className="flex w-full ethereal-bg h-full overflow-hidden">
      ...
      <main className="flex-1 flex flex-col items-center p-0 md:p-4 lg:p-6 relative z-10 h-full overflow-hidden">
        <div className="w-full max-w-3xl h-full flex flex-col md:glass-card md:rounded-[2.5rem] overflow-hidden transition-all duration-500 bg-slate-900/50 md:bg-transparent">
          
          {/* Header - Simetrik Logolar */}
          <header className="flex items-center justify-between px-4 py-3 md:py-6 shrink-0 z-30 border-b border-white/5 bg-slate-900/40 md:bg-transparent backdrop-blur-md">
            ...
            <div className="flex items-center gap-2 md:gap-4 flex-1 justify-center max-w-[70%]">
              <div className="flex items-center gap-1 md:gap-2 shrink-0">
                <div className="bg-white rounded-md p-0.5 w-6 h-6 md:w-10 md:h-10 flex items-center justify-center shadow-lg">
                  <img src={LOGO_URL} alt="Logo" className="w-full h-full object-contain" />
                </div>
                <span className="font-black text-[9px] md:text-[13px] tracking-tight text-white/90 uppercase">NEXTGENLAB</span>
              </div>
              <div className="h-4 w-[1px] bg-white/10 shrink-0 mx-0.5"></div>
              {/* P4C Logosu Boyut Olarak Eşitlendi */}
              <img src={SECOND_LOGO_URL} alt="P4C" className="h-6 md:h-10 object-contain brightness-110 shrink-0" />
            </div>
          ...
          {/* Body - Akıllı Ölçeklendirme */}
          <div className="flex-1 flex flex-col overflow-hidden relative">
            {!activeSessionId ? (
              <div className="h-full flex flex-col items-center justify-center text-center px-4 md:px-12 animate-in fade-in duration-700 overflow-hidden">
                <div className="space-y-8 md:space-y-12 hero-float w-full">
                  <h2 className="text-[2.6rem] sm:text-5xl md:text-8xl font-[1000] text-white tracking-tightest leading-[1.1] md:leading-[1.05] drop-shadow-2xl">
                    Sor. Düşün.<br/> 
                  ...
                  <div className="flex flex-col items-center gap-8 md:gap-12">
                    ...
                    <div className="bg-white/5 inline-flex items-center gap-2 md:gap-3 px-4 md:px-8 py-2 md:py-4 rounded-full border border-white/10 max-w-[95%] overflow-hidden shrink-0">
                    ...
                    <button 
                      ...
                      className="glow-button w-[75%] md:w-auto px-10 md:px-20 py-5 md:py-8 rounded-full text-white font-black text-sm md:text-2xl tracking-tight shadow-2xl transform active:scale-95 transition-all"
                    >
                      Birlikte Keşfedelim
                    </button>
                  </div>
                          ...
                          {msg.data ? (
                            <div className="space-y-8 text-left">
                              ...
                              <div className="p-6 md:p-12 bg-indigo-500/10 border border-indigo-500/20 rounded-2xl md:rounded-[3rem] relative overflow-hidden">
                                ...
                                <label className="text-[8px] md:text-[11px] font-black text-indigo-400 uppercase tracking-widest block mb-3 opacity-60">P4C SORUSU</label>
          ...
          <footer className="px-4 py-5 md:px-12 md:py-12 shrink-0 z-40 bg-slate-900/40 backdrop-blur-md border-t border-white/5">
            ...
            <p className="text-[8px] md:text-[11px] text-white/10 text-center mt-4 uppercase tracking-[0.2em] font-black">

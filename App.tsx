    ...
    <div className="flex w-full bg-white overflow-hidden text-slate-900 selection:bg-indigo-100" style={{ height: 'var(--app-height, 100dvh)' }}>
      ...
      <main className="flex-1 flex flex-col relative min-w-0 bg-[#fbfcfd] mesh-bg h-full overflow-hidden">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar relative z-10 scroll-smooth h-full">
          {!activeSessionId ? (
            <div className="h-full flex flex-col items-center justify-center py-4 md:py-16 px-6 max-w-5xl mx-auto overflow-hidden animate-in fade-in duration-1000">
              
              {/* Logo Bölümü - PC'de daha yukarıda ve slogandan uzaklaştırıldı */}
              <div className="logo-container flex items-center gap-8 md:gap-20 p-6 md:p-12 rounded-[2.5rem] md:rounded-[5rem] shadow-xl transform scale-[0.85] md:scale-110 shrink-0 mb-8 md:mb-24 transition-all duration-500">
              ...
              <div className="space-y-6 md:space-y-16 text-center w-full shrink-0">
                <h2 className="text-3xl md:text-8xl font-[950] text-slate-900 tracking-tightest leading-[1.05]">
                  Sor. Düşün.<br/> 
                  <span className="text-indigo-600">Yapay Zeka ile Keşfet.</span>
                </h2>
                <div className="space-y-4 md:space-y-8">
                  <p className="text-slate-500 text-base md:text-3xl font-medium tracking-tight opacity-80 max-w-[280px] md:max-w-xl mx-auto leading-tight">
                    "Yapay Zeka Çağında Düşünen Nesiller."
                  </p>
                  <div className="inline-block bg-blue-50 border border-blue-100/50 px-4 md:px-12 py-1.5 md:py-3.5 rounded-full shadow-sm">
                    <p className="text-blue-600 text-[8px] md:text-base font-black uppercase tracking-[0.2em] md:tracking-[0.45em] whitespace-nowrap">
              ...
              {/* Öneriler - PC'de aralıklar artırıldı */}
              <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-10 mt-8 md:mt-20 shrink-0 max-w-lg md:max-w-4xl pb-4">
                <button 
                  onClick={() => handleSend("Hayal kurmak sence zihnimizin bir süper gücü müdür?")}
                  className="p-5 md:p-12 text-left bg-white border border-slate-100 rounded-[1.8rem] md:rounded-[3.5rem] hover:border-indigo-300 hover:shadow-2xl transition-all group active:scale-[0.96] shadow-sm"
                >
                ...
                <button 
                  ...
                  className="p-5 md:p-12 text-left bg-white border border-slate-100 rounded-[1.8rem] md:rounded-[3.5rem] hover:border-emerald-300 hover:shadow-2xl transition-all group active:scale-[0.96] shadow-sm"
                >

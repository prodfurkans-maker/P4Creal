...
const App: React.FC = () => {
  ...
  const handleSend = async (customInput?: string) => {
    ...
    // ANLIK TEPKİ: Başlığı beklemeden session oluştur ve mesajı göster
    if (!currentSessionId) {
      const newId = Date.now().toString();
      const newSession: ChatSession = {
        id: newId, 
        title: "Düşünülüyor...", // Geçici başlık
        messages: [userMsg], 
        createdAt: Date.now()
      };
      ...
      // ARKA PLANDA BAŞLIK ÜRET: UI'ı bloklamaz
  ...
  return (
    <div className="flex w-full bg-white overflow-hidden text-slate-900" style={{ height: '100dvh' }}>
      ...
      <main className="flex-1 flex flex-col relative bg-[#fcfdfe] mesh-bg h-full overflow-hidden">
        <header className="h-14 md:h-16 flex items-center justify-between px-6 border-b border-slate-100 bg-white/80 backdrop-blur-md z-30 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="font-black text-slate-900 text-[10px] md:text-base uppercase tracking-tight">
                {activeSession ? activeSession.title : "NextGenLAB"}
              </h1>
            </div>
          </div>
          <button onClick={handleNewChat} className="p-2 text-slate-400 hover:text-indigo-600 active:scale-90 transition-all">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar relative z-10">
          {!activeSessionId ? (
            <div className="h-full flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-700">
              ...
              <div className="w-full max-w-md mt-12 md:mt-16 px-4">
                <button 
                  onClick={() => handleSend("Selam! Bugün birlikte felsefi bir keşfe çıkmaya ne dersin?")}
                  className="w-full p-6 md:p-8 bg-white border border-slate-100 rounded-[2.5rem] shadow-lg hover:shadow-2xl transition-all active:scale-[0.97] group"
                >
                  <p className="text-slate-800 font-bold text-sm md:text-xl group-hover:text-indigo-600 transition-colors">
                      ...
                      <div className="space-y-8 md:space-y-12">
                        {msg.data ? (
                          <div className="space-y-8 md:space-y-12 text-left">
                            ...
                            <div className="p-8 md:p-14 bg-indigo-50/50 rounded-[2.5rem] border border-indigo-100/50">
        ...
        <div className="p-4 md:p-10 bg-gradient-to-t from-white via-white to-transparent z-20 shrink-0">
          <div className="max-w-4xl mx-auto relative">
            <div className="relative flex items-end bg-white border border-slate-200 rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl p-2 md:p-4 pl-6 md:pl-10">
              <textarea
                value={input}

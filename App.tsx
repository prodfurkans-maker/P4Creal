...
const App: React.FC = () => {
  const [sessions, setSessions] = useState<ChatSession[]>(() => {
    const saved = localStorage.getItem('ng_sessions_p4c_v7');
  ...
  useEffect(() => {
    localStorage.setItem('ng_sessions_p4c_v7', JSON.stringify(sessions));
  }, [sessions]);
  ...
  useEffect(() => {
    if (activeSession?.messages && activeSession.messages.length > 0) {
      ...
      if (lastMsg.role === 'assistant' && lastAiMessageRef.current) {
        // Yeni mesaj geldiğinde tam olarak Reflection alanına odaklan
        setTimeout(() => {
  ...
  // Fix: Defined handleNewChat which was missing and causing errors in Sidebar and Header buttons
  const handleNewChat = () => {
  ...
  return (
    <div className="flex w-full ethereal-bg h-full overflow-hidden font-sans text-slate-100">
      ...
      <main className="flex-1 flex flex-col items-center p-0 md:p-2 relative z-10 h-full overflow-hidden">
        <div className="w-full max-w-4xl h-full flex flex-col md:glass-card md:rounded-[1.5rem] overflow-hidden bg-slate-900/40">
          ...
          <header className="flex items-center justify-between px-4 py-3 md:px-8 shrink-0 z-30 border-b border-white/5 backdrop-blur-md">
            <button onClick={() => setIsSidebarOpen(true)} className="p-2 text-white/30 hover:text-white transition-colors">
            ...
            <div className="flex items-center gap-4 flex-1 justify-center">
              <div className="flex items-center gap-2">
                <img src={LOGO_URL} alt="L1" className="w-6 h-6 md:w-8 md:h-8 object-contain bg-white rounded-md p-1 shadow-sm" />
                <span className="font-black text-[9px] md:text-[11px] uppercase tracking-tighter text-white/70">NEXTGENLAB</span>
              </div>
              <div className="h-4 w-[1px] bg-white/10"></div>
              <div className="flex items-center gap-2">
                <img src={SECOND_LOGO_URL} alt="L2" className="w-6 h-6 md:w-8 md:h-8 object-contain bg-white rounded-md p-1 shadow-sm" />
                <span className="font-black text-[9px] md:text-[11px] uppercase tracking-widest text-white/70">P4C</span>
              </div>
            </div>

            <button onClick={() => handleNewChat()} className="p-2 text-white/30 hover:text-white">
            ...
            {!activeSessionId ? (
              <div className="h-full flex flex-col items-center justify-center text-center px-6 animate-in fade-in duration-500">
                <div className="space-y-6 hero-float w-full max-w-lg">
                  <h2 className="text-[2.2rem] md:text-6xl font-[1000] leading-none tracking-tightest">
                    Düşün, Sor.<br/> 
                  ...
                  <div className="flex flex-col items-center gap-6">
                    <button 
                      ...
                      className="glow-button px-10 py-4 rounded-full text-white font-black text-xs md:text-xl tracking-tight transition-transform active:scale-95 shadow-xl"
                    >
                      Keşfe Başla
                    </button>
                  </div>
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar px-3 md:px-10 py-4 space-y-6 md:space-y-12">
                {activeSession.messages.map((msg, idx) => (
                  <div 
                    key={msg.id} 
                    className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-4 duration-300`}
                  >
                    <div className={`max-w-[94%] md:max-w-[85%] px-4 py-3 md:px-10 md:py-8 rounded-2xl md:rounded-[2rem] ${
                      msg.role === 'user' ? 'chat-bubble-user font-bold' : 'chat-bubble-ai'
                    }`}>
                      {msg.role === 'user' ? (
                        <p className="text-base md:text-2xl leading-relaxed">{msg.content}</p>
                      ) : (
                        <div className="space-y-6 md:space-y-8" ref={idx === activeSession.messages.length - 1 ? lastAiMessageRef : null}>
                          {msg.data?.storyContent && msg.data.storyContent.trim() !== "" && (
                            <div className="bg-white/5 p-4 md:p-8 rounded-xl border border-white/5 shadow-inner">
                              ...
                              <p className="text-sm md:text-2xl font-medium leading-relaxed italic text-slate-100">{msg.data.storyContent}</p>
                            </div>
                          )}
                          ...
                          {msg.data?.reflection && (
                            <div className="pl-3 border-l-2 border-indigo-500/40">
                              ...
                              <p className="text-lg md:text-2xl font-black text-white leading-tight">"{msg.data.reflection}"</p>
                            </div>
                          )}

                          {msg.data?.question && (
                            <div className="p-5 md:p-8 bg-indigo-500/10 border border-indigo-500/20 rounded-xl md:rounded-[2rem] relative overflow-hidden shadow-lg">
                              <div className="absolute top-0 left-0 w-1.5 h-full bg-indigo-500"></div>
                              <label className="text-[8px] md:text-[10px] font-black text-indigo-400 uppercase tracking-widest block mb-3">SORGULAMA SORUSU</label>
                              <p className="text-xl md:text-4xl font-black text-white leading-[1.3] tracking-tight">{msg.data.question}</p>
                            </div>
                          )}
                        </div>
                      )}
                ...
                {isLoading && (
                  <div className="flex gap-1.5 text-indigo-400 px-6 items-center opacity-30">
          ...
          {activeSessionId && (
            <footer className="px-4 py-4 md:px-10 md:py-6 shrink-0 z-40 bg-slate-950/20 backdrop-blur-xl border-t border-white/5">
              <div className="relative flex items-center bg-white/5 border border-white/10 rounded-xl md:rounded-full p-1.5 transition-all hover:bg-white/10">
                <textarea
                  ...
                  className="flex-1 max-h-24 md:max-h-32 py-2 px-4 md:px-8 bg-transparent border-none focus:ring-0 text-base md:text-xl font-bold text-white placeholder:text-white/10 resize-none no-scrollbar"
                  rows={1}
                  onInput={(e) => {
                ...
                <button
                  ...
                  className={`p-3 md:p-6 rounded-lg md:rounded-full transition-all ${
              ...
              <p className="text-[7px] md:text-[9px] text-white/10 text-center mt-2 uppercase tracking-[0.4em] font-black italic">NEXTGENLAB P4C ENGINE</p>
            </footer>
          )}
        </div>
      </main>

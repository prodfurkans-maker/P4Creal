    ...
    <div className="flex w-full ethereal-bg h-full overflow-hidden">
      ...
      <main className="flex-1 flex flex-col items-center p-0 md:p-4 lg:p-6 relative z-10 h-full overflow-hidden">
        <div className="w-full max-w-3xl h-full flex flex-col md:glass-card md:rounded-[2.5rem] overflow-hidden transition-all duration-300 bg-slate-900/50 md:bg-transparent shadow-2xl">
          ...
          {/* Header - Enlarger Logos & P4C Label */}
          <header className="flex items-center justify-between px-3 md:px-6 py-4 md:py-8 shrink-0 z-30 border-b border-white/5 backdrop-blur-md">
            ...
            <div className="flex items-center gap-2 md:gap-6 flex-1 justify-center max-w-[85%]">
              {/* Logo 1: NextGenLAB */}
              <div className="flex items-center gap-1.5 md:gap-3 shrink-0">
                <div className="bg-white rounded-xl p-1 w-9 h-9 md:w-14 md:h-14 flex items-center justify-center shadow-2xl">
                  <img src={LOGO_URL} alt="NextGenLAB" className="w-full h-full object-contain" />
                </div>
                <span className="font-black text-[10px] md:text-[14px] tracking-tighter text-white/95 uppercase truncate">NEXTGENLAB</span>
              </div>
              
              <div className="h-6 w-[1px] bg-white/20 shrink-0 mx-0.5 md:mx-1"></div>
              ...
              <div className="flex items-center gap-1.5 md:gap-3 shrink-0">
                ...
                <span className="font-black text-[10px] md:text-[14px] tracking-widest text-white/95 uppercase">P4C</span>
              </div>
            </div>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center px-4 md:px-12 animate-in fade-in duration-500 overflow-hidden">
                <div className="space-y-8 md:space-y-12 hero-float w-full max-w-full">
                  ...
                  <div className="flex flex-col items-center gap-8 md:gap-12 w-full">
                    {/* Slogan Container */}
                    <div className="bg-white/5 inline-flex items-center gap-2 md:gap-3 px-4 md:px-10 py-3 md:py-5 rounded-full border border-white/10 max-w-[98%] overflow-hidden shrink-0">
                      <span className="w-2 md:w-3 h-2 md:h-3 bg-sky-400 rounded-full shadow-[0_0_15px_#38bdf8] animate-pulse shrink-0"></span>
                      <p className="text-[9px] min-[375px]:text-[11px] md:text-base font-black uppercase tracking-[0.1em] md:tracking-[0.4em] text-sky-100 whitespace-nowrap">
                    ...
                    <button 
                      ...
                      className="glow-button w-[80%] md:w-auto px-12 md:px-24 py-5 md:py-9 rounded-full text-white font-black text-sm md:text-2xl tracking-tight shadow-2xl transform active:scale-95 transition-transform"
                    >
                      Birlikte Keşfedelim
                    </button>
                  </div>
                        ...
                        <div className="space-y-8">
                          {msg.data ? (
                            <div className="space-y-10 text-left">
                              <div>
                                <label className="text-[9px] md:text-[12px] font-black text-indigo-300 uppercase tracking-widest block mb-2 opacity-80">Duygu Tasdiki</label>
                                <p className="text-xl md:text-5xl font-black text-white leading-tight">"{msg.data.empathy}"</p>
                              </div>
                              <div>
                                <label className="text-[9px] md:text-[12px] font-black text-sky-300 uppercase tracking-widest block mb-2 opacity-80">Felsefi Işık</label>
                                <p className="text-base md:text-3xl text-slate-200 font-semibold leading-relaxed">{msg.data.suggestion}</p>
                              </div>
                              <div className="p-7 md:p-14 bg-indigo-500/10 border border-indigo-500/20 rounded-2xl md:rounded-[3.5rem] relative overflow-hidden">
                                <div className="absolute top-0 left-0 w-2 h-full bg-indigo-500"></div>
                                <label className="text-[9px] md:text-[12px] font-black text-indigo-400 uppercase tracking-widest block mb-4 opacity-70 italic">P4C SORUSU</label>
                                <p className="text-2xl md:text-6xl font-black text-white leading-[1.1] tracking-tightest">{msg.data.question}</p>
                              </div>
                            </div>
                          ) : <p className="text-base md:text-3xl leading-relaxed font-bold">{msg.content}</p>}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <footer className="px-4 py-6 md:px-12 md:py-12 shrink-0 z-40 bg-slate-900/40 backdrop-blur-md border-t border-white/5">
            <div className="relative flex items-center bg-white/5 border border-white/10 rounded-2xl md:rounded-full p-2 shadow-inner transition-all duration-300">
              <textarea
                value={input} onChange={(e) => setInput(e.target.value)}
                onKeyDown={(e) => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
                placeholder="Düşüncelerini paylaş..."
                className="flex-1 max-h-40 py-4 px-5 md:px-12 bg-transparent border-none focus:ring-0 text-lg md:text-3xl font-bold text-white placeholder:text-white/20 resize-none no-scrollbar"
                rows={1}
                onInput={(e) => {
              ...
              <button
                ...
                className={`p-4 md:p-8 rounded-xl md:rounded-full transition-all flex items-center justify-center ${
                  input.trim() && !isLoading ? 'bg-indigo-600 text-white shadow-2xl hover:bg-indigo-500 active:scale-90' : 'bg-white/5 text-white/5'
                }`}
              >
            ...
            <p className="text-[9px] md:text-[12px] text-white/10 text-center mt-5 uppercase tracking-[0.3em] font-black">NextGenLAB AI – Fikir ve Keşif Ortağın</p>
          </footer>
        </div>
      </main>
    </div>
